...
create_butcher_block_recommendations <- function(data) {
  recommendations <- data |>
    mutate(
      ...
      # From your analysis: High = Meat > 1800, Medium = Meat > 1500
      sales_threshold_meat = 1500,    # Weekly sales threshold for meat
      sales_threshold_seafood = 1850, # Weekly sales threshold for seafood
      ...
      # Dynamic meat length adjustment based on performance
      Meat_Optimal_Length = case_when(
        # If performance is 60% or more below threshold, reduce by 4ft
        Meat_Performance_Pct <= -60 ~ pmax(`Meat Butcher Block` - 4, MIN_MEAT_LENGTH),
        # If performance is 40-60% below threshold, reduce by 3ft
        Meat_Performance_Pct <= -40 ~ pmax(`Meat Butcher Block` - 3, MIN_MEAT_LENGTH),
        # If performance is 20-40% below threshold, reduce by 2ft
        Meat_Performance_Pct <= -20 ~ pmax(`Meat Butcher Block` - 2, MIN_MEAT_LENGTH),
        # If performance is 0-20% below threshold, reduce by 1ft
        Meat_Performance_Pct < 0 ~ pmax(`Meat Butcher Block` - 1, MIN_MEAT_LENGTH),
        
        # If at or above threshold, no recommendations - store performing well
        TRUE ~ `Meat Butcher Block`
      ),
      
      # Dynamic seafood length adjustment based on performance
      Seafood_Optimal_Length = case_when(
        # If performance is 60% or more below threshold, reduce by 4ft
        Seafood_Performance_Pct <= -60 ~ pmax(`Seafood Butcher Block` - 4, MIN_SEAFOOD_LENGTH),
        # If performance is 40-60% below threshold, reduce by 3ft
        Seafood_Performance_Pct <= -40 ~ pmax(`Seafood Butcher Block` - 3, MIN_SEAFOOD_LENGTH),
        # If performance is 20-40% below threshold, reduce by 2ft
        Seafood_Performance_Pct <= -20 ~ pmax(`Seafood Butcher Block` - 2, MIN_SEAFOOD_LENGTH),
        # If performance is 0-20% below threshold, reduce by 1ft
        Seafood_Performance_Pct < 0 ~ pmax(`Seafood Butcher Block` - 1, MIN_SEAFOOD_LENGTH),
    ...
    mutate(
      ...
      # Create recommendation text with performance context
      Meat_Recommendation = case_when(
        ...
        Meat_Length_Change == 0 & Meat_Performance_Pct >= 0 ~ 
        "No recommendations - Store performing well",
      ...
      Seafood_Recommendation = case_when(
        ...
        Seafood_Length_Change == 0 & Seafood_Performance_Pct >= 0 ~ 
        "No recommendations - Store performing well", 
        TRUE ~ "MAINTAIN current length"
      ),
      ...
      # Estimate sales impact based on performance
      Estimated_Meat_Sales_Impact = case_when(
        Meat_Length_Change > 0 ~ Meat_Length_Change * Meat_Sales_per_Foot * 
          (1 + (Meat_Performance_Pct / 100) * 0.5), # Bonus for good performers
        Meat_Length_Change < 0 ~ Meat_Length_Change * Meat_Sales_per_Foot,
        TRUE ~ 0
      ),
      ...
      Estimated_Seafood_Sales_Impact = case_when(
        Seafood_Length_Change > 0 ~ Seafood_Length_Change * Seafood_Sales_per_Foot * 
          (1 + (Seafood_Performance_Pct / 100) * 0.5), # Bonus for good performers
        Seafood_Length_Change < 0 ~ Seafood_Length_Change * Seafood_Sales_per_Foot,
        TRUE ~ 0
      ),
      ...
      # ROI calculation
      Weekly_ROI = case_when(
        abs(Total_Cost_Change) > 0 ~ (Total_Sales_Impact * 4) / abs(Total_Cost_Change),
        TRUE ~ 0
      ),
      
      # Priority score based on impact magnitude and performance gap
      Priority_Score = case_when(
        (abs(Meat_Performance_Pct) >= 40 | abs(Seafood_Performance_Pct) >= 40) & 
        abs(Total_Cost_Change) > 500 ~ "HIGH",
        (abs(Meat_Performance_Pct) >= 20 | abs(Seafood_Performance_Pct) >= 20) & 
        abs(Total_Cost_Change) > 200 ~ "MEDIUM",
        abs(Total_Cost_Change) > 0 ~ "LOW",
        TRUE ~ "NO CHANGE"
      ),
...
recommendation_summary <- butcher_recommendations |>
  summarise(
    ...
    # Financial impact
    Total_Cost_Impact = round(sum(Total_Cost_Change), 0),
    Total_Sales_Impact = round(sum(Total_Sales_Impact), 0),
    Net_Impact = round(sum(Total_Sales_Impact) + sum(Total_Cost_Change), 0),
...
cat("=== DYNAMIC BUTCHER BLOCK OPTIMIZATION SUMMARY ===\n")
print(recommendation_summary)

# Show stores with significant changes
significant_changes <- butcher_recommendations |>
  filter(abs(Meat_Length_Change) >= 2 | abs(Seafood_Length_Change) >= 2) |>
  select(`Store ID`, Meat_Performance_Pct, Seafood_Performance_Pct, 
         Meat_Recommendation, Seafood_Recommendation, 
         Total_Cost_Change, Priority_Score)

if(nrow(significant_changes) > 0) {
  cat("\n=== STORES WITH SIGNIFICANT CHANGES (2+ feet) ===\n")
